...
from src.utils import PositionalEncoding, SinusoidalPosEmb, prob_mask_like
...
class DanceDecoder(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        self.null_cond_embed = nn.Parameter(torch.randn(1, 1, latent_dim))
        ...
        self.dancer_proj = nn.Linear(dancer_emb_dim, latent_dim)
    ...
    def guided_forward(self, x, cond_embed, dancer_embed, times, guidance_weight):
        unc = self.forward(x, cond_embed, times, dancer_embed, cond_drop_prob=1)
        conditioned = self.forward(x, cond_embed, times, dancer_embed, cond_drop_prob=0)
    ...
    def forward(
        ...
        null_cond_embed = null_cond_embed.expand_as(cond_tokens)
        ...
        if dancer_embed is not None:
            # project & add to cond_hidden (style-ish)
            dancer_hidden = self.dancer_proj(dancer_embed)  # [B, D]
            # TODO: try more complex fusion?
            cond_hidden = cond_hidden + dancer_hidden

            # also treat dancer as an extra token the decoder can attend to
            dancer_token = dancer_hidden.unsqueeze(1)  # [B, 1, D]
            cond_tokens = torch.cat([cond_tokens, dancer_token], dim=1)
