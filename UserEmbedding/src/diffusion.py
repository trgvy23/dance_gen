...
from data.quaternion import ax_from_6v, quat_slerp
from data.smpl_skeleton import skeleton_render
...
class GaussianDiffusion(nn.Module):
    ...
    def model_predictions(
        self, x, cond, dancer_embed, t, weight=None, clip_x_start=False
    ):
        weight = weight if weight is not None else self.guidance_weight
        model_output = self.model.guided_forward(x, cond, dancer_embed, t, weight)
        maybe_clip = (
    ...
    def p_mean_variance(self, x, cond, dancer_embed, t):
        # guidance clipping
        ...
        x_recon = self.predict_start_from_noise(
            x, t=t, noise=self.model.guided_forward(x, cond, dancer_embed, t, weight)
        )
    ...
    @torch.no_grad()
    def p_sample(self, x, cond, dancer_embed, t):
        b, *_, device = *x.shape, x.device
        model_mean, _, model_log_variance, x_start = self.p_mean_variance(
            x=x, cond=cond, dancer_embed=dancer_embed, t=t
        )
    ...
    def p_sample_loop(
        ...
        for i in tqdm(reversed(range(0, start_point))):
            ...
            x, _ = self.p_sample(x, cond, dancer_embed, timesteps)
    ...
    @torch.no_grad()
    def ddim_sample(self, shape, cond, dancer_embed, **kwargs):
        ...
        for time, time_next in tqdm(time_pairs, desc="sampling loop time step"):
            ...
            pred_noise, x_start, *_ = self.model_predictions(
                x, cond, dancer_embed, time_cond, clip_x_start=self.clip_denoised
            )
    ...
    @torch.no_grad()
    def long_ddim_sample(self, shape, cond, dancer_embed, **kwargs):
        batch, device, total_timesteps, sampling_timesteps, eta = (
            shape[0],
            self.betas.device,
            self.n_timestep,
    ...
    def p_losses(self, x_start, cond, t, dancer_embed=None):
        noise = torch.randn_like(x_start)
        x_noisy = self.q_sample(x_start=x_start, t=t, noise=noise)

        # reconstruct
        x_recon = self.model(
            x_noisy, cond, t, dancer_embed, cond_drop_prob=self.cond_drop_prob
        )
    ...
    def loss(self, x, cond, dancer_embed=None, t_override=None):
        batch_size = len(x)
        ...
        return self.p_losses(x, cond, t, dancer_embed)

    def forward(self, x, cond, dancer_embed=None, t_override=None):
        return self.loss(x, cond, dancer_embed, t_override)
